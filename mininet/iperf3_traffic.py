...
def reset_environment(wait=2):
    ...
    print("Resetting environment (killing stale client-side iperf3, clearing tc)...")
    for host in net.hosts:
        if host.name == SERVER_HOST:
            continue  # never kill the server's iperf3 -s listeners
...
def stop_all_traffic():
    print("Stopping all client-side iperf3 processes...")
    for host in net.hosts:
        if host.name == SERVER_HOST:
            continue  # never kill the server's iperf3 -s listeners
        host.cmd("pkill -9 -f iperf3 2>/dev/null")
